...
`settings.log...` 屬性已從伺服器腳本中移除，並移至 `local/kubejsdev.properties` 檔案。默認情況下，它不會隨包一起發送，但您可以將 saveDevPropertiesInConfig 更改為 true，以將檔案保存到 `kubejs/config/dev.properties`。
