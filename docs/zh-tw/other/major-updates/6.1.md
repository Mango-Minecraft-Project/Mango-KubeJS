...
- **`NetworkEvents.fromServer`** 和 **`NetworkEvents.fromClient`** 已合併為 **`NetworkEvents.dataReceived`**，它將根據腳本類型處理來自相應端的傳入資料。
...
- **將 kubejs/logs 檔案從 .txt 重命名為 .log** - 因此您現在可以在 VSCode 等中進行格式化。
- **修復了資源包和資料包的順序** - 用戶添加的資源包和資料包現在將位於 KubeJS 生成的包之上，因此您應該可以使用它們更改紋理和其他內容。
- **添加了從 kubejs/data 和 kubejs/assets 加載 .zip 檔案的功能** - 您只需將 .zip 檔案放入該資料夾，它將強制加載（在 KubeJS 之上，位於用戶包之下）
...
- **添加了 `/kubejs packmode` [mode] 命令** - 與上述相同，但您完全不需要處理檔案。
...
- **修復了資料生成問題** - KJS 不應再在開發環境中阻止資料生成關閉遊戲。[我們真的是生活在一個奇蹟的時代！](https://www.youtube.com/watch?v=TiWWvDrIpIE)
